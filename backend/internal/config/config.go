...
// struct tags “ = reflection = undertanding structure of data during runtime
type ServerConfig struct {
	Port               string   `koanf:"port" validate:"required"`
	ReadTimeout        int      `koanf:"read_timeout" validate:"required"`
	WriteTimeout       int      `koanf:"write_timeout" validate:"required"`
	IdleTimeout        int      `koanf:"port" validate:"required"`
...
func LoadConfig() (*Config, error) {
	...
	k := koanf.New(".")
	// prefixing env in runtime is useful for us to identify the source when being deployed into ec2 instances
	// or somewhere else (easy debug)
