...
// Loggerservice managaer new relic integration
...
func NewLoggerService(cfg *config.ObservabilityConfig) *LoggerService {
	...
	if cfg.NewRelic.LicenseKey == "" {
		fmt.Println("No licening key for new relic bruv")
		return service
	}
	...
	// Add debug logging only if explicitly enabled (generally for debug level)
...
func NewLoggerWithService(cfg *config.ObservabilityConfig, loggerService *LoggerService) zerolog.Logger {
	...
	if cfg.IsProduction() && cfg.Logging.Format == "json" {
		...
		// Development mode using console writer
		consoleWriter := zerolog.ConsoleWriter{Out: os.Stdout, TimeFormat: "2006-01-02 15:04:05"}
		writer = consoleWriter
	}

	// Note moment: New Relic log forwarding is now handled automatically by zerologWriter integration
...
// Traces in newrelic are considered transactions
